...
_pq_datas, _pq_bins, _pq_hidden = collect_all('PyQt6')

# WebEngine требует явного сбора — PyInstaller не находит DLL автоматически
try:
    _we_datas, _we_bins, _we_hidden = collect_all('PyQt6.QtWebEngineCore')
except Exception:
    _we_datas, _we_bins, _we_hidden = [], [], []
try:
    _wew_datas, _wew_bins, _wew_hidden = collect_all('PyQt6.QtWebEngineWidgets')
except Exception:
    _wew_datas, _wew_bins, _wew_hidden = [], [], []
...
a = Analysis(
    [r'{ROOT / 'main.py'}'],
    pathex=[r'{ROOT}'],
    binaries=_pq_bins + _we_bins + _wew_bins,
    datas=[
        {datas_str},
    ] + _pq_datas + _we_datas + _wew_datas,
    hiddenimports=[
        ...
        'PyQt6.QtWebChannel',
        'socks',  # PySocks — обязателен для SOCKS прокси
        'psutil', 'wmi', 'win32api', 'win32con',
        'openpyxl', 'reportlab',
    ] + _pq_hidden + _we_hidden + _wew_hidden,
    hookspath=[],
    hooksconfig={{}},
    runtime_hooks=[],
    excludes=[
