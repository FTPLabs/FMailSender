## [5.2.4] — 2026-06-26
  ...
  ### Fix
  - **Фальшивые «невалидные» аккаунты** (`core/sender.py`): когда прокси-IP заблокирован SMTP-сервером и прямое подключение также не проходит авторизацию, аккаунты больше не помечаются «Невалидный» (красный). Теперь отображается «Ошибка прокси» (оранжевый) — это честнее: невозможно подтвердить неправильный пароль, если прокси заблокирован. Сообщение об ошибке стало более информативным (различает ошибку авторизации 534/535, подсказывает про App-пароль).
  - **Медленная проверка** (`core/sender.py`): при SMTP-блокировке IP прокси теперь пропускается Step 3 (12 попыток через заблокированный прокси на разных портах). Экономит 6-8 секунд на аккаунт — скорость верификации значительно возрастает.
