...
### Fixed
- Экран активации теперь показывает фактическую причину отказа из локального ядра, а не общий текст «Ключ недействителен».
- Добавлен безопасный диагностический лог ошибок активации в `core.log`.
- Windows release smoke-test проверяет полный packaged запрос активации к production API.

## [7.3.4] — 2026-08-17

### Fixed
- Исправлен бесконечный экран «Ожидание сервера»: readiness определяется по незащищённому `/api/health`, а не по лицензируемому `/api/status`.
- Кнопка перезапуска теперь вызывает фактический Electron relaunch через IPC.
- Добавлен диагностический `%APPDATA%\FMailSender\core.log` и расширена проверка packaged backend в Windows CI.

## [7.3.3] — 2026-08-17

### Fixed
- Заменён нестабильный `fork` на явный Electron-as-Node запуск backend-ядра с проверкой ресурсов и строгим health-check.

## [7.3.2] — 2026-08-16

### Fixed
- В release workflow добавлен Windows smoke-тест: собранный Electron `.exe` обязан запустить локальное ядро и вернуть health-ответ с ожидаемой версией до публикации релиза.

## [7.3.1] — 2026-08-16

### Fixed
- В Electron-ядре включена строгая проверка TLS-сертификатов для SMTP-подключений и проверок аккаунтов.
- Ограничен параллелизм отправки и проверки прокси до четырёх соединений.
- Прокси и получатели переведены в сессионные данные; они не записываются на диск и не восстанавливаются после перезапуска.
- Самотест Node-ядра больше не инициализирует хранилище и периодическую проверку лицензии.
- Синхронизированы версии Python, Node, Electron, UI и Tauri; обновлены устаревшие проверки ядра.
