  ...
  class SendingEngine:
      ...
      async def run_campaign(self, recipients: List[Recipient], template: EmailTemplate) -> List[SendResult]:
          ...
          async def send_with_semaphore(recipient: Recipient) -> SendResult:
              # Ждём снятия паузы ДО захвата семафора — иначе все слоты
              # будут заблокированы на время паузы и рассылка встанет.
              ...
              async with self._semaphore:
                  ...
                  # Пауза после N писем — sleep ВНУТРИ семафора (только один слот занят)
                  if self.stats["sent"] % self.config.pause_after_n == 0 and not self._stopped:
                      self._log(f"\u23f8 Пауза {self.config.pause_duration_sec}с после {self.stats['sent']} писем")
                      await asyncio.sleep(self.config.pause_duration_sec)

                  # Случайная задержка между письмами
          ...
          tasks = [send_with_semaphore(r) for r in recipients]
