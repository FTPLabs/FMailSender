"""
FMailSender core sending engine v2.6.0.
Fixes: thread-safe increment_sent, MIME structure, SendingEngine API for screen_sending.py.
"""
...
async def test_smtp_connection(account: SmtpAccount) -> tuple[bool, str]:
    ...
    try:
        if account.use_ssl:
            smtp = aiosmtplib.SMTP(hostname=account.host, port=account.port, use_tls=True, start_tls=False, timeout=20)
        else:
            smtp = aiosmtplib.SMTP(hostname=account.host, port=account.port, use_tls=False, start_tls=account.use_tls, timeout=20)
        await smtp.connect()
...
class SendingEngine:
    """
    Async campaign engine compatible with screen_sending.py.
    ...
    async def _send_one(
        ...
        async with sem:
            if self.stop_event.is_set():
                return SendResult(recipient_email=recipient.email, success=False, error="Отменено", account_used=account.email)
            personalized = template.personalize(recipient)
            msg = _build_message(account, recipient, personalized)
            if not _HAS_AIOSMTPLIB:
                return await asyncio.get_event_loop().run_in_executor(None, self._send_sync, account, recipient, msg)
            try:
                if account.use_ssl:
                    await aiosmtplib.send(msg, hostname=account.host, port=account.port, username=account.email, password=account.password, use_tls=True, start_tls=False, timeout=30)
                else:
                    await aiosmtplib.send(msg, hostname=account.host, port=account.port, username=account.email, password=account.password, use_tls=False, start_tls=account.use_tls, timeout=30)
                account.increment_sent()
                return SendResult(recipient_email=recipient.email, success=True, account_used=account.email, message_id=msg.get("Message-ID", ""))
            except Exception as exc:
                return SendResult(recipient_email=recipient.email, success=False, error=str(exc), account_used=account.email)
