"""
  ...
  Поддерживает offline grace period 72ч.
  """
  import base64
  import ctypes
  import hashlib
  ...
  # FIX: JWT secret must match server default (config.py JWT_SECRET default)
  _JWT_SECRET_DEFAULT = "fmsnd-jwt-2026-X9K2M7B4Q3F8W1T5R6Y9P0"
  ...
  def check_license() -> Tuple[bool, Optional[LicenseInfo], str]:
      ...
      try:
          ...
          if time.time() - last_online < OFFLINE_GRACE_HOURS * 3600:
              ...
              # Создаём info из кэшированных данных
              try:
                  payload_unverified = jwt.decode(token_val, options={"verify_signature": False})
                  info = LicenseInfo(payload_unverified)
