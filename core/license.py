...
def security_check() -> None:
    ...
    # VM-проверка удалена: Hyper-V/WSL2/VMware используются легитимно
...
def activate_license(key: str, progress_callback=None) -> Tuple[bool, str]:
    ...
    # Демо-ключ — активируется оффлайн
...
def check_license() -> Tuple[bool, Optional[LicenseInfo], str]:
    ...
    """
    # Демо-режим: мгновенный доступ без ключа
    if DEMO_MODE:
    ...
    try:
        ...
        logger.error(f"License token parse error: {e}")
        return False, None, "Ошибка чтения лицензии. Активируйте снова."
